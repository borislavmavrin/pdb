...
//            pdb2.DivCompress(17-pattern.size(), true);
            std::vector<int> pattern = {1, 2, 3, 4, 0};
            ...
            LexPermutationPDB<MNPuzzleState<4, 4>, slideDir, MNPuzzle<4, 4>> pdb2(ts, t, pattern);
            pdb2.BuildAdditivePDB(t, std::thread::hardware_concurrency());
//            pdb2.DeltaCompress(ts, t, true);
//            pdb2.DivCompress(17-pattern.size(), true);
            std::cout << pdb2.PDB.Size() << '\n';
            std::cout << pdb2.PDB.Get(0) << '\n';
            std::cout << pdb2.PDB.Get(1) << '\n';
            pdb2.GetStateFromPDBHash(524159, s);
            std::cout << s << '\n';
            uint64_t COUNT = pdb2.PDB.Size();
            uint64_t value = 0;
            std::ofstream pdbfile;
            pdbfile.open ("/Users/borislav/Downloads/pdb.txt");
            for (uint64_t x = 0; x < COUNT; x++)
            {
                value = pdb2.PDB.Get(x);
